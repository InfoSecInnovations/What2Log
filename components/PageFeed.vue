<template>
  <div v-if="pages && pages.length" class="page-feed">
    ...
    <div class="blog-posts">
      <PageCard v-for="page of pages" :key="page.slug" :article="page" :link="`/${directory}/${page.slug}/`" />
    </div>
...
export default {
  ...
  async fetch() {
    let query = this.$content(`${this.$i18n.locale}/${this.directory}`)
    if (this.query) query = query.where(this.query)
